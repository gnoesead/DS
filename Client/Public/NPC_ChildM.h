...
class CNPC_ChildM final : public CNPC
{
public:
	enum ANIM {
		ANIM_CRY = 0,
		...
		ANIM_SIT_SPEAK = 13, ANIM_SIT_KNEE_SPEAK = 16,

		ANIM_SPEAK = 19,
		ANIM_SPEAK_NORLAM = 22,

		ANIM_STAGGER_B = 25,
		ANIM_STAGGER_F = 26,

		ANIM_STAND = 27, ANIM_STAND_GONGSON = 28,
...
private:
	CNPC_ChildM(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CNPC_ChildM(const CNPC_ChildM& rhs);
	virtual ~CNPC_ChildM() = default;
...
public:
	static CNPC_ChildM* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
