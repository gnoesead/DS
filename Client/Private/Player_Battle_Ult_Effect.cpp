...
void CPlayer_Battle_Ult_Effect::LateTick(_double TimeDelta)
{
	...
	if (m_Pre_Ult_Stack != m_Ult_Stack) {
		...
		if (m_Pre_Ult_Stack < m_Ult_Stack) {
			...
			CSoundMgr::Get_Instance()->Play_Sound_Channel(szSoundFile, CSoundMgr::UI_GAUGE, 0.3f);
			m_Is_Stack_Up = true;
		}
		...
		if (m_Pre_Ult_Stack > m_Ult_Stack) {
			...
			CSoundMgr::Get_Instance()->Play_Sound_Channel(szSoundFile, CSoundMgr::UI_GAUGE, 0.43f);
