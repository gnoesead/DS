...
CNPC_Female::CNPC_Female(const CNPC_Female& rhs)
	: CNPC(rhs)
{
}

HRESULT CNPC_Female::Initialize_Prototype()
{
...
HRESULT CNPC_Female::Initialize(void* pArg)
{
	...
	if (FAILED(Read_Animation_Control_File("NPC_Female.bin")))
	{
		MSG_BOX("Failed to AnimData Read : NPC_Female");
		return E_FAIL;
	}
...
HRESULT CNPC_Female::Render_ShadowDepth()
{
...
void CNPC_Female::EventCall_Control(_double dTimeDelta)
{
...
void CNPC_Female::Animation_Control_Quest(_double dTimeDelta)
{
	...
	{
		m_pModelCom->Set_Animation(ANIM_SPEAK_SHY);
	}
...
{
	/* for.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_NPC_Female"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSG_BOX("Failed to Add_Com_Model : CNPC_Female");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed to Add_Com_Shader : CNPC_Female");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed to Add_Com_Transform : CNPC_Female");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed to Add_Com_AABB : CNPC_Female");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed to Add_Com_OBB : CNPC_Female");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed to Add_Com_Sphere : CNPC_Female");
		return E_FAIL;
	}
...
HRESULT CNPC_Female::SetUp_ShaderResources()
{
...
CNPC_Female* CNPC_Female::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CNPC_Female* pInstance = new CNPC_Female(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CNPC_Female");
...
CGameObject* CNPC_Female::Clone(void* pArg)
{
	CNPC_Female* pInstance = new CNPC_Female(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CNPC_Female");
		Safe_Release(pInstance);
	}
