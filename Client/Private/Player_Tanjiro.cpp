...
void CPlayer_Tanjiro::Animation_Control_Battle_Jump(_double dTimeDelta)
{
	...
	if (m_Moveset.m_Down_Battle_Jump_Attack)
	{
		...
		else
		{
			...
			//m_pModelCom->Set_EarlyEnd(30, true, 0.99f);
			JumpStop(0.95);
...
void CPlayer_Tanjiro::Animation_Control_Battle_Attack(_double dTimeDelta)
{
	...
	if (m_Moveset.m_Down_Battle_Combo)
	{
		...
		else
		{
			...
			if (23 == iCurAnimIndex)
				m_pModelCom->Set_EarlyEnd(23, true, 0.4f);
			if (25 == iCurAnimIndex)
	...
	m_pModelCom->Set_EarlyEnd(23, false, 0.4f);
...
void CPlayer_Tanjiro::Animation_Control_Battle_Dash(_double dTimeDelta)
{
	...
	//더블스텝
	Go_Dir_Deceleration(dTimeDelta, 98, 5.0f * m_fScaleChange * m_fMoving_Ratio, 0.15f * m_fScaleChange, m_Moveset.m_Input_Dir);
	Go_Dir_Deceleration(dTimeDelta, 100, 5.0f * m_fScaleChange * m_fMoving_Ratio, 0.15f * m_fScaleChange, m_Moveset.m_Input_Dir);
}
...
#pragma region Dmg_Upper
	...
	Go_Dir_Constant(dTimeDelta, 125, 0.5f * m_fDmg_Move_Ratio, AtkDir);
