...
PS_OUT  PS_Main(PS_IN _In)
{
	...
	vector vSourDiffuse = g_DiffuseTexture[0].Sample(LinearSampler, _In.vTexCoord * 10.f);
	vector vDestDiffuse = g_DiffuseTexture[1].Sample(LinearSampler, _In.vTexCoord * 10.f);
	vector vMask = g_MaskTexture.Sample(LinearSampler, _In.vTexCoord * 1.f);

	Out.vDiffuse = vDestDiffuse * vMask.r + vSourDiffuse * (1.f - vMask.r);
	Out.vDiffuse.a = 1.f;
